...
def preprocess_image(image_path, input_shape):
    ...
    image = (image - 0.5) / 0.5  # Mean subtraction and division (if needed)
...
if __name__ == "__main__":
    ...
    input_shape = (1, 3, 256, 256)  # Change to your model's input shape
    ...
    print("Inference output len:", len(h_output))

    # Print the shape of each output tensor to identify the correct one
    for i, output in enumerate(h_output):
        print(f"Output {i} shape: {output.shape}")

    # Assuming the first output contains the landmarks
    landmark_output = h_output[9]
    im = draw_landmarks(image_path, landmark_output)
    cv2.imwrite('im.jpg', im)

    # print("Landmark output:", landmark_output)
    print("Landmark output shape:", landmark_output.shape)
